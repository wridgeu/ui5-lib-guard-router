...
FLP-specific smoke coverage lives in `test/flp/flp-preview.e2e.ts`. It verifies that the app boots inside the shell preview, exposes the FLP runtime state, and shows the FLP dirty-state confirmation when cross-app navigation is attempted from a dirty page.
