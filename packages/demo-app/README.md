...
**Enhanced homepage (CDM mode):** The FLP preview uses `enhancedHomePage: true`, which bootstraps the shell via CDM (`container.init('cdm')`) instead of the classic `sandbox.js`. This provides a more complete UShell service layer -- including `ShellNavigationHashChanger`, `CrossApplicationNavigation`, and dirty-state infrastructure -- that closely matches production FLP behavior. Requires UI5 >= 1.123.0.
